...
class Car{
	...
	인스턴스를 초기화 할 수 있다. 따라서 확장성이 없는 형태의
	메서드이다.	 */
	void initialize() {
		//Car클래스의 멤버변수를 초기화한다.
		carModel = "람보르기니";
	...
	//'운전하다' 기능을 구현한 멤버메서드
	void drive() {
		/*owner는 객체이므로 이름을 출력하기 위해 '변수명.name'과
		같이 기술해야한다.*/
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		/*
		자동차1 객체 생성 : 매개변수가 없는 초기화메서드를 호출하여
			람보르기니로 초기화한다.
		*/
		Car car1 = new Car();
		car1.initialize();
		...
		//자동차2 객체 생성
		/* 초기화 메서드를 호출하는 대신 멤버변수에 직접
		접근하여 인스턴스를 초기화한다. 이 경우 인스턴스를
		...
		//자동차3 객체 생성
		/* 매개변수가 있는 초기화 메서드를 사용하여
		인스턴스를 생성한다. 이 경우 매개변수를 통해 다양한 값을
		전달할 수 있으므로 여러형태의 객체를 생성할 수 있다.
