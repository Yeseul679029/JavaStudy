...
public class Ex06DeveloperDefine {
	...
	public static void main(String[] args) {
		...
		try {
			...
			throw하면 여기에서 예외처리한다. 즉 메서드를 호출한
			지점에서 예외처리 하는 방식이다.*/
			int age = readAge();
			System.out.println("당신이 나이는  "+age+" 입니다.");
		}
	...
	public static int readAge() throws AgeErrorException{
		...
		int inputAge = 0;
		try {
			/* 나이 입력시 문자를 잘못 입력하는 경우에 대해 예외
			처리를 하고있따. 이 부분은 Java에서 미리 정의해놓은
			예외처리 클래스를 사용한다.*/
			inputAge = sc.nextInt();
		}
		...
		/* 만약 나이를 음수로 입력하는 경우 JVM이 감지하지 못하
		므로 이때는 if문을 통해 판단해야한다. 개발자가 정의한
		예외객체를 생성한 후 직접 throw 한다.*/
		if(inputAge<0) {
			AgeErrorException ex = new AgeErrorException();
			/* 예외객체를 throw하면 즉시 에러가 발생된다. 이를 반드시
			catch해야되고, 그렇지 않으면 JVM에 의해 예외객체가
			throw 되는것과 동일하게 프로그램이 비정상 종료된다.*/
			throw ex;
		}
		return inputAge;
	}
