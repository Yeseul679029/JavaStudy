...
/*
지역변수(Local variable)
-변수는 사용범위(Scope)를 가지고 있고, 해당 지역내에서만 사용할 수있다.
...
scope로 표현 어떻게 판단하느냐 중괄호로 판단
정렬만 제대로 되어있으면 넓은지역과 좁은지역을 판단하기 쉬워짐
 */
...
//		int num =9; //아래 코드에서 에러가 발생됨.
		...
		if~else 문은 하나의 문장이 맞지만 지역은 다르다 
		true와 false일때 각각 다른 지역이 실행되기때문에 
		똑같은 변수를 만들수있다.
		 */
		if(scope) {
			int num = 1;
		...
		/*if문의 블럭에서 선언된 변수num은 해당 블럭을 벗어나는 순간 메모리에서 
	...
	/*main에서 선언한 변수 num은 메모리에 아직 상주하고 있지만 해당 
	 메서드는 크기가 동일한 서로 다른 지역에 정의되었으므로
	 같은 이름의 변수를 선언할 수 있다.
