...
public class E05For {
	...
	public static void main(String[] args) {
		...
		System.out.println("\n1~10사이 2의배수의합(방법1):"+total);
		
		/*
		방법2 : if문없이 초기값을 0부터 시작하여 2씩 증가시킨다.
		 */
		total = 0;
		...
		System.out.println("\n1~10사이 2의배수의합(방법2)"+total);
		
		//for문으로 무한루프를 만들때는 (;;)와 같이 표현한다.
		//for문을 이렇게 쓰는경우는 거의 없지만 문법적으로 가능하다~ 정도.
		int a =1;
		for(;;) {
		...
		for(int j=0; j<=5;j++);
		{
			System.out.println("어랏...나는누구?여긴어디");
		}
...
//		System.out.println("위 for문에서 선언한 변수j="+j);
		...
		초기값을 외부로 증감식을 내부쪽으로 빼낼 수도있지만 이렇게 쓰는경우는
		거의 없으며 문법적으로 가능하다 정도로 보면 된다.
		 */
		int i=0;
		for(; i<=5;i++) {
