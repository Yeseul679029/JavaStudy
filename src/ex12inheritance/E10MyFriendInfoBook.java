...
class HighFriend extends Friend{
	...
	오버라이딩1 : 고딩친구의 전체정보를 출력하기 위해 부모에서 정의한 
		메서드를 super를 통해 호출하고, 자식에서 확장한 변수를 
		추가해서 출력한다.*/
	@Override
	public void showAllData() {
...
//기능을 담당하는클래스 매니저클래스 핸들러클래스
/* 해당 프로그램에서 기능을 담당하는 클래스로 핸들러 혹은 매니저
클래스라고 부른다. */
class FriendInfoHandler{
	
	//멤버변수 
	/* 상속관계에서 부모클래스의 인스턴스 배열을 생성한다. 부모로
...
//		System.out.println("addFriend 호출됨");
		...
		/* 더이상 실행할 문장이 없으므로 해당 메서드는 메모리에서 소멸
		되고, 호출한 지점인 main메서드로 돌아간다.	 */
	}/////end of addFriend
...
//		System.out.println("showSimpleData 호출됨");
		...
		일일히 확인한 후 강제형변환(다운캐스팅) 해야한다.
			오버라이드 처리가 되지않으면 다운캐스팅이 일어나야한다
		for(int i=0; i<numOfFriends;i++) {
