...
형변환(즉 상속관계가 있으면)이 가능하면 true를 반환한다.
...
public class E09InstanceOf {
	...
	필요하다.
	-instanceof 연산자를 통해 자식타입으로 형변환이 가능한지 판단한
	후 true를 반환하면 다운캐스팅을 진행한다.
	 */
	//매개변수는 Box 타입
	...
	아래와 같이 각각의 타입에 따른 모든 메서드를 정의해야한다.
	
	static void wrapBox(Box b) {}
	static void wrapBox(PaperBox b) {}
	static void wrapBox(GoldPaperBox b) {}
	
	상속관계가 더 깊어진다면 메서드는 점점 늘어나게되어 코드는 더욱
	더 복잡해지게 될것이다. 따라서 부모로 자식을 참조할 수 있는것은
	개발자에게 편리함을 가져다준다.
	 */
