...
public class DeChild extends DeParent {
	...
	//generate > using field 에서 위에 카테고리를 둘다 전달한다고 체크
...
//		System.out.println(name+"이(가) 공부한다.");
	...
	/* 오버라이딩 아님 : 부모클래스에서 private멤버로 선언
		 ...
		 될수없다. 해당 메서드는 자식쪽에서 새롭게 확장한 메서드가
		 된다.
		오버라이딩이 되려면 접근이 가능해서 그걸 타고 가야하는데 
		private이라면 불가능하다
	*/
	private void eat() {
	...
	*/
	...
	/*
	static 클래스명으로 접근가능해
	Method영역에 올라가 <- 처음부터 끝까지 이게특징
	static(정적) 메서드는 오버라이딩의 대상이 될수 없다.
	클래스 외부(메서드 영역)에 미리 로드되므로 오버라이딩 
	되지않고, 호출할때 각 클래스명을 통해 사용해야한다.
	 */
