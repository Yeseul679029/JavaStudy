...
public class E02ArrayAndMethod {
	...
	public static void main(String[] args) {
		/*배열을 선언과 동시에 초기화한다. 초기값의 갯수를 통해
		 ...
		 배열변수인 arr에 할당하게된다.*/
		int[] arr = {1,2,3,4,5};
		//배열변수를 선언하였지만 아직은 참조할 배열 인스턴스가없는 상태
		int[] ref;//참조 인스턴스가 없이 스택에 변수만 만들어둔상태
		...
		전달하는 것으로 배열 자체가 전달되지 않는다. 아래 출력문에서는
		...
		다시 반환받았으므로 결국 동일한 배열을 참조하게 되어 아래
	...
	/*
	매개변수로 전달된 배열의 참조값을 ar로 받게되므로 해당
	메서드에서도 동일한 배열을 참조할 수 있게된다. 즉 main메서드에서
	생성한 배열과 동일한 배열을 참조하게된다.
	 */
