/*
패키지선언부 : 현재 작성하는 클래스의 위치를 결정하기 위한
	선언으로 해당 클래스를 특정 패키지 하위에 묶어서 생성한다는
	의미이다.
 */
package ex09package;
...
public class E01PackageMain {
	...
	public static void main(String[] args) {
		...
		방법2 : 하나의 클래스는 import 하고, 다른 하나는 풀
			패키지 경로를 기술한다.
		 */
		...
		/* static의 장점
		static으로 선언된 메서드는 new를 통해 인스턴스 생성
		없이 클래스명으로 직접 메서드를 호출할 수 있다.
		Java에서 제공하는 클래스 중 대표적으로 Math를 예로 들수 있다.
		난수생성을 위해 별도의 인스턴스 생성없이 
		클래스.함수명
		...
		/* 클래스 작성시 패키지명까지 모두 입력한 상태로 사용하는것이 가능하다.*/
		System.out.println(
