...
메모리를 할당한 후 이를 프로그램 전체에서 공유해서 사용하고자할때
쓰는 패턴이다.
하나만 딱 생성해서 프로그램 전체에 공유하고싶을때 

작성방법
1.생성자의 접근지정자를 private으로 선언한다.
2.그러면 클래스 외부에서는 생성자에 접근할 수 없으므로 new를
통해 인스턴스 생성이 불가능해진다.
3.클래스 내부에 멤버변수를 static으로 선언하여 인스턴스를 미리 
생성해 놓는다.
...
class NoSingleTone{
	...
	선언하지 않으면 클래스외부 혹은 다른 패키지에서는 호출할 수
	없기때문에 new를 통해 인스턴스를 생성할 수 없기때문이다. */
	public NoSingleTone() {}
}
//싱글턴 패턴이 적용된 클래스 정의
class SingleTone{
...
//		SingleTone st1 = new SingleTone();//에러발생
		...
		해당 메서드 호출을 통해 싱글톤 인스턴스의 참조값을 얻어올
		수 있다.
		 */
		//여기에서 얻어오는 참조값과...
