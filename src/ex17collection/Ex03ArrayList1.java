...
//		list.add(5,"소방차ㅋㅋ"); //에러발생됨
		...
		/*
		포함여부확인 (검색을하는정도는 불가능하고 단순하게 있다 없다 정도만 알수있다.)
		...
		방법1 : 인덱스로 삭제
			remove(인덱스) : 인덱스를 통해 삭제를 진행한 후
				삭제가 완료되면 해당 인스턴스를 반환한다. 삭제
				후에는 인덱스가 자동으로 부여된다. 삭제성공시
				해당 인스턴스를 반환하므로 삭제 후 즉시 확인할
				수 있다.
		 */
		...
		방법2 : 
		 */
		//indexof()를 통해 해당 인스턴스의 인덱스를 찾은 후
		//삭제한다.
