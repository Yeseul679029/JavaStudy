...
-객체가 순서없이 저장된다.
-객체의 중복저장을 허용하지 않는다.
-기본클래스(String, Integer)인 경우 별도의 처리없이 중복이
	제거된다.
-하지만 개발자가 정의한 클래스라면 hashCode(), equals()메서드를
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		/* Object를 기반으로 set컬렉션을 생성했으므로 모든 객체를
		저장할 수 있다. */
		...
		Teacher vatTeacher = new Teacher("김봉두", 55, "체육");
		
		/* add저장에 성공하면 true를 반환하게되어있다.
...
//		System.out.println(set.add(new String("개발자")) ? "저장성공":"저장실패");
		...
		/*
		이터레이터를 통한 객체 출력 : 순서없이 저장되므로 출력의
			순서도 정할 수 없다.
		 */
		...
		while(itr.hasNext()) {
			...
			변환되어 저장되므로 우리가 직접 정의한 클래스에 대해서
			만 instanceof를 통해 타입을 확인한 후 다운캐스팅하여
			출력한다. 만약 다운캐스팅을 하지않으면 자식멤버에
			접근할 수 없으므로 getName()을 호출할 수 없다.*/
		...
		/*
		객체삭제 : set컬렉션은 인덱스가 없으므로 객체의 참조값을
			통해서만 삭제할 수 있다.
		 */
		System.out.println(set.remove(varString2) ? 
					"varString2 삭제 성공": "varString2 삭제실패");
		
		/*
		전체삭제 : List컬렉션과 동일하다. 
		 */
		set.clear();
		//이미다 삭제가 된후라 false가 뜬다?
