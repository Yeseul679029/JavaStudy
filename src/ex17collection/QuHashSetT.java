...
해당 어벤져스의 정보를 출력하고 없으면 "해당 영웅은 없어요ㅜㅜ" 라는 메시지 출력해야 한다. 
...
메소드 오버라이딩을 통해 hero4 는 입력되지 않도록 AvengersT 클래스를 업데이트 하시오.
...
class AvengersT {
	...
	public AvengersT(String name, String heroName, String weapon) {
		super();
		this.name = name;
	...
	public String toString() {
		return "AvengersT [본명=" + name + ", 닉네임=" + heroName + ", "
				+ "능력=" + weapon +"]";
	}
	...
	public int hashCode() {
		...
		int retCode2 = Objects.hash(this.name,
				this.heroName,
				this.weapon);
...
public class QuHashSetT {
	...
	public static void main(String[] args) {
		...
		HashSet<AvengersT> set = new HashSet<AvengersT>();

		AvengersT hero1 = new AvengersT("토니스타크", "아이언맨", "Mark-48 수트");
		AvengersT hero2 = new AvengersT("스티브로져스", "캡틴아메리카", "비브라늄 방패");
		AvengersT hero3 = new AvengersT("브루스배너", "헐크", "강한피부&점프");
		AvengersT hero4 = new AvengersT("토니스타크", "아이언맨", "Mark-48 수트");
		...
		//이터레이터 객체를 생성한다.
		Iterator<AvengersT> it = set.iterator();
		//반환할 객체가 있는지 확인한다.
		while(it.hasNext()) {
			/* 이터레이터는 next() 메서드를 통해 객체를 출력한다.
			한번 호출할때마다 다음 객체로 이동하게 되므로 루프내에서
			한번이상 호출하면 예외가 발생할 수 있어 반드시 아래와
			같이 사용해야 한다.*/
			AvengersT av = it.next();
			//해당 루프의 객체와 이름을 비교한다.
