...
: 메인쓰레드와 Working쓰레드(개발자가 정의한 쓰레드)를 일컫은 것으로
메인쓰레드가 종료되어도 계속 실행되며, Dead상태가 되어야만 실행이
종료되는 쓰레드를말한다. 해당 쓰레드는 main() 메서드가 종료된
이후에도 계속 실행되는것을 볼수있다.
 */
class NormalThread extends Thread{
	...
	쓰레드의 이름을 지정할 수 있다. 이때 지정된 이름은 getName()
	을 통해 반환받을 수 있다.*/
	...
	/*
	쓰레드 동작중 비정상적으로 종료될때 InterruptedException
...
종료될때까지는 프로그램이 종료된것이 아니다.
부로 프로그램의 보고역할로 배경음악재생, 로그자동저장 등의 업무를
처리한다. 프로그램이 종료될때 함꼐 종료되므로 무한루프로 구현한다.
 */
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		/* 첫번째 독립쓰레드 생성 및 시작. 우선순위를 별도로 부여
		하지 않으므로 5로 지정된다. 쓰레드 인스턴스 생성시 이름을
		부여하는 방법으로 아래 2가지가 있다.*/
