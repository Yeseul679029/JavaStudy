...
/* DTO(Data Transfer Object) : 데이터를 저장하고 전송하기 위한
 	용도의 객체를 말한다. VO(Value Object)로 표현하기도한다.
...
class PersonalNumberStorageExt extends PersonalNumberStorageAbs{
	...
	/* 상속을 통해 추상메서드를 포함하게 되므로 반드시 아래와같이
	오버라이딩 해야한다. 이는 필수사항이며 하지 않을경우 에러가
	발생한다. */
	@Override
	...
	public String searchPersonalInfo(String juminNum) {
		...
		for(int i=0; i<numOfPerInfo; i++) {
			//주민번호가 일치하는지 확인한다. private이라 getter사용
			if(juminNum.compareTo(personalArr[i].getJuminNum())==0) {
				//일치하는 경우 이름을 반환한다.
				return personalArr[i].getName();
				/* DTO객체의 멤버변수는 private으로 선언되었
				으므로 외부에서는 접근이 불가능하다. 따라서 
				getter를 통해 접근해야한다.*/
			}
		}
