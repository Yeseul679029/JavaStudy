...
강제사항이 아닌 선택사항이다. 즉 필요한 경우에만 오버라이딩하면 된다.
...
/* 추상클래스2 : 추상메서드를 포함한 클래스. 추상메서드를 멤버로
	포함한 클래스는 반드시 abstract로 선언해야한다. 그렇지 않으면
	에러가 발생한다.*/
...
되므로 해당 클래스도 abstract로 선언해야한다. 단, 이경우 해당
클래스로도 인스턴스를 생성할 수 없게된다. 따라서 객체생성을 위해서는
부모의 추상메서드를 자식쪽에서 오버라이딩 하면된다.
 */
class AbsClassChild2 extends AbsClass2{
	/* 오버라이딩은 부모쪽의 메서드를 가리고, 새로운 기능을 재정의
	하는것이므로 해당 클래스는 추상메서드를 포함하지 않은 상태가된다.
	따라서 인스턴스 생성이 가능해진다. 
...
//		AbsClass1 absClass1 = new AbsClass1();
		...
		AbsClassChild1 abssClassChild1 = new AbsClassChild1();
		
		/* 추상클래스는 인스턴스 생성은 할수없지만, 참조변수로는
		사용할 수 있다.*/
		...
		/* 추상클래스 내에 선언된 정적메서드는 오버라이딩의 대상이
		될 수 없으므로 static의 기본규칙을 따른다. 별도의 생성 절차
		없이 클래스명으로 바로 접근할 수 있다.*/
