...
import java.text.SimpleDateFormat;//간단한 날짜의 서식.
import java.util.Calendar;
import java.util.Date;

public class E05Date {

	/* Calendar클래스를 통해 얻은 인스턴스를 초로 변환한 후 
	매개변수로 전달된 날짜서식으로 변경한 String을 반환한다. */
	public static String myDateFormat(Calendar c, String formatStr) {
		/* getTime() : 서버의 표준시인 1970-01-01 0시0분0초 부터 현재
		까지의 시간을 초 단위로 반환해준다. 이를 타임스템프라고 한다. */
		Date date = c.getTime();
	...
	public static void main(String[] args) {
		...
		/*
		Calendar 클래스는 대표적인 싱글톤 객체이므로 new로 인스턴스를
		생성할 수 없고, 유틸리티 메서드를 통해 참조값을 얻어온다.
		 */
		Calendar calendar = Calendar.getInstance();
		
		/* 월을 표현하는 Month, 분을 표현하는 Minute의 이니셜이
		동일하기 때문에 구분을 위해 뭘은 대문자, 분은 소문자를 사용
		한다.*/
...
//		calendar.add(Calendar.DATE, -3); //3일전 과거를 구하려면 음수값입력
		calendar.add(Calendar.MONTH, 2); //2개우러후
		calendar.add(Calendar.YEAR, 1); //내년(1년후)
		
		//앞에서 add()를 3번 호출했으므로 결과는 아래와같이 출력된다.
...
//			System.out.println(formatToday+" "+formatFuture);
			...
			/* getTime()으로 얻어오는 시간은 13자리의 정수값인데
			10자리까지는 순수한 초(Sec)이고, 11~13자리는
			밀리세컨즈를 의미한다. 즉 100으로 나눠서 뒤의 3자리를
			제거하면 순수한 초가 반환된다. 또한 하루는 60*60*24
			즉 86400초 이므로 아래와 같이 남은 날짜를 구할 수
			있다. */
